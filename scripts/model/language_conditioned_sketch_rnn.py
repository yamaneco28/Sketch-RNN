...
class LanguageConditionedSketchRNN(nn.Module):
    ...
    def loss(self, x, y, mean, std, label, eps=1e-10, weight_mse=100, weight_language=100):
        x = x.to(self.device)
        y = y.to(self.device)
        ...
        # language loss
        text_features = self.clip_model.get_text_features(input_ids=label.to(self.device))
